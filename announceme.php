...
//Admin-Actions
	...
	function announcemeAdminHead() {
		?><link rel="stylesheet" type="text/CSS" href="<?php bloginfo('url'); ?>/wp-content/plugins/announceme/admin.css" />
<script>
...
if(isset($_GET['submit'])) { $do = false;
 ...
 if($do) {
  if($_POST['id']=="new") {
   mysql_query("INSERT INTO ".$wpdb->prefix."announceme (`announce`,`cat`,`img`,`active`,`author`,`date`,`c1`,`c2`,`c3`,`dsa`) VALUES ('".$_POST['announce']."', '".$_POST['cat']."', '".$img[$_POST['cat']]."', '".$act."', '".get_current_user_id()."', NOW(), '".$_POST['c1']."', '".$_POST['c2']."', '".$_POST['c3']."','".$_POST['dsa']."')") or die(mysql_error());
   ?><script>top.location.href = 'admin.php?page=AnnounceME&msg=Announcement was created successfully';</script><?php
  } else {
   mysql_query("UPDATE ".$wpdb->prefix."announceme SET `announce` = '".$_POST['announce']."',`cat` = '".$_POST['cat']."',`img` = '".$img[$_POST['cat']]."',`active` = '".$act."',`author` = '".get_current_user_id()."', `c1` = '".$_POST['c1']."', `c2` = '".$_POST['c2']."', `c3` = '".$_POST['c3']."', `dsa` = '".$_POST['dsa']."' WHERE id = '".$_POST['id']."'");
...
//Fontend
	...
	function announcemeHead() { global $wpdb;
		$q1 = mysql_query("SELECT * FROM ".$wpdb->prefix."announceme WHERE active = '1'");
                $t = 0;
		while($q2=mysql_fetch_array($q1)) { if(!isset($_COOKIE['announceme-'.$q2['id']])) { $t += 41; } }
		?><style>body {amargin-top: <?php echo $t; ?>px;} .admin-bar {margin-top: <?php echo (28+$t); ?>px;} #announceme {position: fixed; z-index: 100; top: 0; left: 0; right: 0;} .admin-bar #announceme {top: 28px;} .announceme {float: left; padding: 4px 0; width: 100%;} .announceme-cat-1 {background: #55ff55; border-bottom: 1px solid #00d200;} .announceme-cat-2 {background: lightYellow; border-bottom: 1px solid #E6DB55;} .announceme-cat-3 {background: #c64a4a; border-bottom: 1px solid #960000;} .announceme-cat {float: left; margin-left: 15px;} .announceme-msg {float: left; padding: 9px; color: #000; text-shadow: 0px 1px 1px #fff; font: 13px Arial; font-weight: bold;} .announceme-close {float: right; padding: 10px 8px 4px 8px; margin-top: 2px; font: 16px Courier; font-weight: bold; color: #000; cursor: pointer; margin-right: 5px;} .announceme-close:hover {text-shadow: 0px 0px 3px #000;} .announceme-del {float: right; padding: 10px; color: #000; font: 10px Verdana; cursor: pointer;}</style><script src="wp-content/plugins/announceme/jquery.js" type="text/JavaScript"></script><script>url = "<?php echo str_replace('http://','',get_bloginfo('url')); ?>";</script>
<script src="wp-content/plugins/announceme/announceme.js" type="text/JavaScript"></script><?php
	}
...
<?php if($q2['dsa']=="1") { ?>
 <div class="announceme-del"><input type="checkbox" /> Don't show this again</div>
<?php } ?>
</div>
	<?php } } ?></div><?php } ?>
